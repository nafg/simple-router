...
object Lub {
  implicit def lub[T]: Lub[T, T, T] = new Lub[T, T, T] {
    override val left = identity
    override val right = identity
  }
}
 ...
 */
class PathRoute[R <: RouteType, +A](val path: Path[R], val route: R#Route[A])
                                   (implicit mapRoute: CanLiftRouteMapping[R]) extends Sitelet[A] {
  override def run: PartialFunction[Location, A] = path run route

  override def map[B](f: A => B): PathRoute[R, B] = new PathRoute[R, B](path, mapRoute(f)(route))
}
...
class RouteSeq[+A](val pathRoutes: Seq[PathRoute[_, A]]) extends Sitelet[A] {
  override def run: PartialFunction[Location, A] =
    pathRoutes.foldLeft(PartialFunction.empty[Location, A])(_ orElse _.run)
