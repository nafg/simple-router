package object simplerouter {
  ...
  implicit class ArgPathOps[A](arg: Arg[A])
    extends Path.PathRouteOpsBase[A >>: RConst]
      with Path.PathComponentOpsBase[A >>: RConst] {
    val path = PArg(arg, PNil)
  }

  implicit class ParamPathOps[A](param: Param[A])
    extends Path.PathRouteOpsBase[Option[A] >>: RConst] with Path.PathParamOpsBase[Option[A] >>: RConst] {
    val path = PParam(param, PNil)
  }

  implicit class ParamsPathOps[A](params: Params[A])
    extends Path.PathRouteOpsBase[List[A] >>: RConst] with Path.PathParamOpsBase[List[A] >>: RConst] {
